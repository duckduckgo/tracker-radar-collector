...
function rebuildIndex(outputPath, screenshotPaths) {
    const bodyContent = screenshotPaths.map(
        (item) =>
            `<p>${item.url}</p><a href='${item.path}'><img src='${item.path}' alt='${item.url}' loading='lazy' style='max-width: 800px; max-height: 400px' /></a>`,
    );
...
function createMetadataHTML(outputPath, { startTime, crawlTimes, fatalError, numberOfCrawlers, regionCode, successes, failures, urls }) {
    ...
    const crawlStats = crawlTimes.reduce(
        (stats, siteTime) => {
            stats.sites++;

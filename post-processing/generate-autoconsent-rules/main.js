...
async function processCookiePopupsForSite(globalParams, { finalUrl, initialUrl, collectorResult, existingRules }) {
    ...
    await Promise.all(
        rulesToOverride.map(async (rule) => {
            console.log(`${finalUrl}: overriding rule ${rule.name}`);
            updatedRuleFiles.push(
                await writeRuleFiles({
                    rule,
                    url: finalUrl,
                    rulesDir,
                    testDir,
    ...
    await Promise.all(
        newRules.map(async (ruleToWrite) => {
...
async function main() {
    ...
    program
        .description(
            `Generate autoconsent rules from a crawl directory
    Example:
